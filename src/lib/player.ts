...
import { isNativePlayback, startPlaybackNotification, updatePlaybackNotification } from './nativePlayback';
...
export interface PlayerState {
  ...
  eq: EQState;
  /** Feature (Loudness normalization): when true, playback gain is nudged
   *  per-song by Song.gainDb -- see _effectiveVolume(). */
  ...
  playbackRate: number;
  /** Feature (Speed/pitch control): true = pitch stays natural regardless
   *  of speed (the common case); false = pitch shifts with speed, like a
   *  slowed-down/sped-up tape. */
...
class Player {
  ...
  private _state: PlayerState = {
    ...
    crossfadeSeconds: 0, eq: EQ_FLAT, normalizeVolume: false,
    sleepTimerEndsAt: null, sleepTimerEndOfTrack: false,
    playbackRate: 1, preservePitch: true,
  };
  ...
  /** Feature (Notification gaps): fires once when the sleep timer actually
   *  pauses playback (either mode -- countdown or end-of-track), so a
   *  backgrounded app can raise an OS notification instead of the person
   *  only discovering it stopped whenever they next check their phone. */
  onSleepTimerFired: (() => void) | null = null;
  ...
  private _effectiveVolume(): number {
    if (this._state.muted) return 0;
    return this._state.volume * this._normalizationFactor();
  }

  // Feature (Loudness normalization): converts the current song's analyzed
  // gainDb into a linear multiplier on top of the user's volume slider --
  // 1.0 (no change) whenever normalization is off, the song hasn't been
  // analyzed yet (gainDb undefined), or gainDb is 0.
  private _normalizationFactor(): number {
    if (!this._state.normalizeVolume) return 1;
    const gainDb = this._state.currentSong?.gainDb;
    if (!gainDb) return 1;
  ...
  // Feature (Speed/pitch control): applies the current rate/pitch-
  // preservation settings to one <audio> element. `preservesPitch` is
  // unprefixed in modern Chrome/Edge/Firefox but still needs the
  // webkit-prefixed form for Safari, hence the fallback chain -- there's no
  // single standard property name across browsers yet.
  private _applyPlaybackSettings(a: HTMLAudioElement) {
    a.playbackRate = this._state.playbackRate;
    const el = a as HTMLAudioElement & { preservesPitch?: boolean; webkitPreservesPitch?: boolean; mozPreservesPitch?: boolean };
    el.preservesPitch = this._state.preservePitch;
    el.webkitPreservesPitch = this._state.preservePitch;
    el.mozPreservesPitch = this._state.preservePitch;
  }
  ...
  private _patch(patch: Partial<PlayerState>) {
    ...
    // Feature (Persistent playback notification): only re-syncs on an
    // actual isPlaying flip, not on every single patch (position updates
    // alone fire many times a second via timeupdate and would otherwise
    // spam the native side with redundant identical notification updates).
    if (patch.isPlaying !== undefined && patch.isPlaying !== wasPlaying) this._syncNativePlaybackNotification();
  ...
  async loadSong(song: Song, autoplay = true) {
    ...
    try {
      ...
      audio.src = url;
      audio.load();
      this._applyPlaybackSettings(audio);
      this._applySongEQ(song);
  ...
  setNormalizeVolume(on: boolean) { this._patch({ normalizeVolume: on }); this._applyVolumeSteadyState(); }

  // Feature (Speed/pitch control): applied to both underlying <audio>
  // elements immediately (not just the active one) so whichever one is
  // mid-crossfade or about to be prefetched-into also picks up the change,
  // instead of only the currently-audible element.
  setPlaybackRate(rate: number) {
    const clamped = Math.min(2, Math.max(0.5, rate));
    this._patch({ playbackRate: clamped });
    this._applyPlaybackSettings(this.audioA);
    this._applyPlaybackSettings(this.audioB);
  }
  setPreservePitch(preserve: boolean) {
    this._patch({ preservePitch: preserve });
    this._applyPlaybackSettings(this.audioA);
    this._applyPlaybackSettings(this.audioB);
  }
  ...
  // Feature (Per-song EQ override): applies a song's customEQ (if it has
  // one) directly to the filter gain nodes, WITHOUT going through
  // setEQAll/_patch -- the whole point is that the app's persisted global
  // EQ setting (what SettingsPanel shows and saves) must stay exactly what
  // the user last set it to. Falls back to the current global curve when
  // the song has no override, which also correctly restores the global
  // curve on the very next song after an overridden one.
  private _applySongEQ(song: Song) {
    this._ensureAudioGraph();
    const eq = song.customEQ ?? this._state.eq;
    EQ_BANDS.forEach((band, i) => {
      const clamped = clampEQ(eq[band.key]);
      if (this.eqFilters[i]) this.eqFilters[i].gain.value = clamped;
    });
  ...
  private async _beginCrossfade() {
    ...
    newAudio.src = url;
    newAudio.load();
    this._applyPlaybackSettings(newAudio);
    this._applySongEQ(nextSong);
  ...
  patchCurrentSong(updated: Song) {
    if (this._state.currentSong?.id === updated.id) {
      ...
      // Feature (Per-song EQ override): if the song being edited is the one
      // currently playing, apply its (possibly just-changed) EQ curve live
      // instead of waiting for the next song load to pick it up.
      this._applySongEQ(updated);
    }
  }
  ...
  // Feature (Background playback): the EQ/crossfade graph routes every
  // element's audio through an AudioContext (see _ensureAudioGraph) --
  // sourceA/B -> gain -> filters -> ctx.destination. That means audible
  // output depends entirely on the context staying 'running', which a
  // plain <audio> element wouldn't need. Mobile browsers (iOS Safari in
  // particular, but also Chrome/Android under aggressive power saving)
  ...
  private _wireBackgroundResume() {
    ...
    const resumeIfNeeded = () => {
      if (this.ctx && this.ctx.state === 'suspended' && this._state.isPlaying) {
        this.ctx.resume().catch(() => { /* will retry on the next trigger */ });
      }
    };
  ...
  private _updateMediaSessionMetadata(song: Song) {
    ...
    if (song.albumArtData) {
      ...
      // Feature (Notifications: richer lock-screen art): the OS media
      // notification/lock-screen widget picks whichever listed size best
      // matches its own layout instead of always using the largest -- we
      // only actually have one decoded image, but listing it under several
      // size labels (same underlying blob URL each time) means it still
      // gets picked for smaller notification-shade layouts instead of
      // being skipped for not matching any entry.
      const mime = song.albumArtMime || 'image/jpeg';
      artwork = ['96x96', '192x192', '256x256', '384x384', '512x512'].map((sizes) => ({ src: url, sizes, type: mime }));
    }
    try {
    ...
    // Feature (Persistent playback notification): reuses the same blob URL
    // just created above rather than re-encoding the art a second time --
    // nativePlayback.ts converts it to a data: URL before handing it to
    // the native side, since a blob: URL only means anything inside this
    // WebView's own process.
    this._syncNativePlaybackNotification();
  }

  // Feature (Persistent playback notification): the single place that
  // pushes the current song + play/pause state to the native foreground-
  // service notification (see lib/nativePlayback.ts). Called both here
  // (whenever the song/art changes) and from _patch() (whenever isPlaying
  // toggles) so the notification's Play/Pause button and ongoing/dismissible
  // state stay in sync with actual playback, not just track changes.
  private _nativeNotificationStarted = false;
  private _syncNativePlaybackNotification() {
    ...
    const meta = {
      ...
      artist: song.artist,
      album: song.album,
      artworkUrl: this._mediaSessionArtUrl ?? undefined,
      isPlaying: this._state.isPlaying,
    };
    if (!this._nativeNotificationStarted) {
      this._nativeNotificationStarted = true;
      startPlaybackNotification(meta);
    } else {
      updatePlaybackNotification(meta);
    }
  }
  ...
  private _updateMediaSessionPosition() {
    ...
    try {
      navigator.mediaSession.setPositionState?.({ duration, playbackRate: this._state.playbackRate, position: Math.min(currentTime, duration) });
